      ...
      const timeoutId = setTimeout(() => {
        if (!isResolved) {
          ...
          extractorLogger.warn(
            `[TimeoutDecorator] 解析超时 (${actualTimeoutMs / 1000}秒): ${filePath}`
          );
...
export function withLogging(
  ...
  const { logStart = false, logEnd = false, logError = true, prefix = 'Extractor' } = config;
...
export function withRetry(
  ...
  const { maxRetries = 3, retryDelay = 1000, exponentialBackoff = false } = config;
