...
export class WalkerHandler {
  ...
  private handleFilesBatch(files: any[]): void {
    ...
    for (const file of files) {
      ...
      if (
        state.getWalkerTotalCount() % 100 === 0 ||
        Date.now() - this.lastProgressUpdateTime > 1000
      ) {
  ...
  private handleWalkingComplete(message: any): void {
    ...
    if (this.options.walkerCompletedCountRef.value > this.options.totalWalkerTasks) {
      log.error(
        '[Walker] 错误: 完成计数 ({}) 超过总任务数 ({})，这不应该发生！',
        this.options.walkerCompletedCountRef.value,
        this.options.totalWalkerTasks
      );
  ...
  private adjustMemoryAfterWalkerComplete(): void {
    ...
    if (state.getTaskQueueLength() > 0) {
      ...
      if (stats.totalCount > 0) {
        ...
        context.log.info(
          '[智能内存调整]平均文件大小: {}MB, 新内存限制: 老生代={}MB, 新生代={}MB',
          avgFileSizeMB.toFixed(2),
          newLimits.oldGen,
          newLimits.youngGen
        );
        ...
        if (!state.cancelFlag && state.isScanning) {
          setTimeout(() => {
            // 【再次检查】防止在延迟期间扫描已结束
            if (!state.cancelFlag && state.isScanning) {
              const restartedCount = workerPool.restartIdleWorkers(
                newLimits.oldGen,
                newLimits.youngGen
              );
