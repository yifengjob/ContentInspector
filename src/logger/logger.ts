...
// 【新增】检测是否在 Worker 线程
const isWorkerThread = typeof process !== 'undefined' && typeof process.send === 'function';
...
export function setupFileLogger(logDir: string): void {
  ...
  const timeStr = beijingTimeStr.replace(/\//g, '-').replace(/ /g, 'T').replace(/:/g, '-');
   ...
   */
  const redirectConsoleMethod = (originalFn: (...args: any[]) => void, levelPrefix: string) => {
...
export function createLogger(config: string | LogConfig): Logger {
  ...
  const enableFile =
    typeof config === 'object'
      ? (config.enableFile ?? DEFAULT_LOG_CONFIG.enableFile)
      : DEFAULT_LOG_CONFIG.enableFile;
  const enableFrontend =
    typeof config === 'object'
      ? (config.enableFrontend ?? DEFAULT_LOG_CONFIG.enableFrontend)
      : DEFAULT_LOG_CONFIG.enableFrontend;
  // 【移除】enableMemory 已不再使用

  // 【新增】获取日志级别配置
  const fileLevel =
    typeof config === 'object'
      ? (config.fileLevel ?? DEFAULT_LOG_CONFIG.fileLevel)
      : DEFAULT_LOG_CONFIG.fileLevel;
  const frontendLevel =
    typeof config === 'object'
      ? (config.frontendLevel ?? DEFAULT_LOG_CONFIG.frontendLevel)
      : DEFAULT_LOG_CONFIG.frontendLevel;
