...
![Prettier](https://img.shields.io/badge/Prettier-3.8.3-F7B93E.svg)
...
- **SVG 图标**：vite-plugin-svg-icons 2.0.1（内联 SVG 雪碧图）
- **文档预览**：@vue-office/docx 1.6.3、@vue-office/excel 1.7.14、@vue-office/pdf 2.0.10、@vue-office/pptx 1.0.1

### 后端技术
- **框架**：Electron 22.3.27（兼容 Windows 7）
- **语言**：Node.js + TypeScript 5.9.3
- **多线程**：Worker Threads（CPU 密集型任务隔离）
- **文件系统**：fs, walkdir 0.4.1
- **序列化**：JSON
...
| `@vue-office/docx` | 1.6.3 | Word 文档原生预览 |
| `@vue-office/excel` | 1.7.14 | Excel 表格原生预览 |
| `@vue-office/pdf` | 2.0.10 | PDF 文档原生预览 |
| `@vue-office/pptx` | 1.0.1 | PowerPoint 演示文稿原生预览 |
| `vue-demi` | 0.14.6 | Vue 2/3 兼容层 |
| `electron` | 22.3.27 | 桌面应用框架（兼容 Win7） |
| `typescript` | 5.9.3 | 类型系统 |
| `vite` | 6.4.2 | 前端构建工具 |
| `pdfjs-dist` | 3.11.174 | PDF 逐页解析（Mozilla 官方引擎） |
| `exceljs` | 4.4.0 | Excel 文件读写（xlsx, xls） |
| `xlsx` | 0.20.3 | SheetJS，快速解析 Excel（高性能） |
| `word-extractor` | 1.0.4 | Word/PPT 文档解析（docx, doc, wps） |
| `walkdir` | 0.4.1 | 目录遍历（异步 I/O） |
| `trash` | 9.0.0 | 文件回收站操作（跨平台） |
| `chrono-node` | 2.9.1 | 时间处理（日期识别） |
| `fflate` | 0.8.2 | ZIP 解压（替代 adm-zip，高性能） |
| `iconv-lite` | 0.7.2 | 编码转换（GBK/UTF-8，RTF 解码） |
| `sax` | 1.6.0 | XML 解析（流式处理） |
| `expr-eval` | 2.0.2 | 表达式求值（搜索表达式） |
| `readable-stream` | 3.6.2 | Node.js 流 API polyfill |
| `buffer` | 6.0.3 | Buffer polyfill（浏览器兼容） |
| `events` | 3.3.0 | EventEmitter polyfill |
| `process` | 0.11.10 | process polyfill（浏览器兼容） |
...
| 库名 | 版本 | 用途 |
|------|------|------|
| `@types/node` | 20.19.39 / 24.12.2 | Node.js 类型定义 |
| `@types/sax` | 1.2.7 | sax 类型定义 |
| `@vitejs/plugin-vue` | 5.2.4 | Vite Vue 插件 |
| `vue-tsc` | 2.2.12 | Vue TypeScript 检查 |
| `vite-plugin-svg-icons` | 2.0.1 | SVG 雪碧图插件 |
| `fast-glob` | 3.3.3 | 快速文件匹配 |
| `electron-builder` | 24.13.3 | Electron 打包工具 |
| `concurrently` | 8.2.2 | 并行运行多个命令 |
| `cross-env` | 10.1.0 | 跨平台环境变量设置 |
| `wait-on` | 9.0.5 | 等待资源就绪（开发模式） |
| `eslint` | 10.4.0 | 代码质量检查 |
| `typescript-eslint` | 8.59.3 | TypeScript ESLint 插件 |
